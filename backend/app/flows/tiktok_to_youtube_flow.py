...
from app.services.tiktok_download_service import tiktok_download_service, TikTokDownloadError
...
class TikTokToYouTubeFlow:
    JOB_TYPE = "tiktok_to_youtube"
    STORAGE_PREFIX = "tiktok_downloads"
    ...
    async def run_process(self, job_id: str):
        ...
        async with async_session() as db:
            ...
            try:
                ...
                job.progress = 10
                await db.commit()
                await self._send_telegram(job, "📥 Đang tải video từ TikTok...")
                self._update_state(progress=10)
                tt_meta = await tiktok_download_service.download_with_metadata(
                    job.source_url, job_dir
                )
                source_path = tt_meta.get("filepath") or source_path
                job.source_file_path = source_path
                job.status = "running"
                ...
                logger.exception("TikTok process failed job_id=%s", job_id)
                raise
    ...
    async def _run_after_glossary(self, source_metadata: dict, job_id: str):
        ...
        if transcript and transcript.strip():
            ...
            meta = seo_metadata_service.generate(
                "Nội dung video từ TikTok. " + source_metadata.get("title", ""),
                source_metadata,
                glossary_items,
            )
        ...
        async with async_session() as db:
            ...
            try:
                ...
                result = youtube_upload_service.upload_video(
                    ...
                    title=job.ai_title or "Video từ TikTok",
